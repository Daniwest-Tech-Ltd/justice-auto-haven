...
const CarDetails = () => {
  ...
  const carTitle = `${car.year} ${car.make} ${car.model} for Sale in Nairobi | Justice Ultimate`;
  const carPrice = car.price.toLocaleString();
  const seoDescription = `Buy this ${car.year} ${car.make} ${car.model} for KSh ${carPrice} at Justice Ultimate Automobiles. Features: ${car.engine}, ${car.transmission}, ${car.fuel_type}. Located in ${car.yard_location || 'Nairobi'}. Up to 90% financing available.`;

  // Generate Google Schema Markup for the Vehicle
  const structuredData = {
    ...
    "mileageFromOdometer": {
      ...
      "value": car.mileage ? parseInt(car.mileage.replace(/\D/g,'')) || 0 : 0,
      "unitCode": "KMT"
    },
    "vehicleIdentificationNumber": car.stock_id || "PENDING", // Should ideally be a real VIN
    "image": images[0] || "https://www.justiceultimateautomobiles.com/images/company-logo.png",
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-brand-red selection:text-white font-sans antialiased overflow-x-hidden text-slate-900 pb-20">
      <SEO
        title={carTitle}
        description={seoDescription}
        keywords={`${car.make} for sale Kenya, ${car.model} price Nairobi, used ${car.make} ${car.model} Kenya, Lipa Mdogo Mdogo ${car.make}`}
        image={images[0]}
        url={window.location.href}
      />

      {/* Inject Structured Data into the Head for Google Search Rich Results */}
      <script type="application/ld+json">
        {JSON.stringify(structuredData)}
      </script>

      {/* Floating WhatsApp Conversion Button */}
      <div className="fixed bottom-6 right-6 z-[90] sm:bottom-8 sm:right-8 group">
        <a
          href={whatsappLink}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center h-14 w-14 sm:h-16 sm:w-16 bg-[#25D366] text-white rounded-full shadow-[0_10px_30px_rgba(37,211,102,0.4)] hover:scale-110 transition-transform duration-300 relative"
          onClick={() => {
            // Optional: You can track this click in GA4 here
            console.log("WhatsApp Conversion Triggered:", car.stock_id);
          }}
        >
          <span className="absolute -top-12 -right-4 bg-white text-slate-900 text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-lg border border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
            Response in 12 mins
          </span>
          <MessageCircle className="h-7 w-7 sm:h-8 sm:w-8" />
          {/* Signal Ping */}
          <span className="absolute inset-0 rounded-full animate-ping border-2 border-white/50 opacity-20 pointer-events-none" />
        </a>
      </div>
      ...
      <div className="container max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8">
        {/* Top Info Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-100 pb-8 mb-8 sm:mb-12">
          <div className="space-y-3 w-full md:w-auto">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="text-xl sm:text-2xl font-black text-brand-red tracking-tight flex items-baseline gap-1">
                <span className="text-xs font-bold text-slate-400">KSh</span>
                {car.price.toLocaleString()}
              </div>
            </div>
            <h1 className="text-xl sm:text-3xl md:text-4xl font-black tracking-tight text-slate-900 uppercase leading-tight">
              {car.make} {car.model} {car.year} {car.engine} {car.fuel_type}
            </h1>
          </div>
          <div className="text-left md:text-right space-y-2 w-full md:w-auto pt-4 md:pt-0 border-t md:border-none border-slate-100">
            <p className="text-lg md:text-xl font-black text-brand-red uppercase tracking-tight">
        ...
        <div className="grid gap-8 lg:gap-12 lg:grid-cols-12 mb-20">
            ...
            <div className="relative group rounded-xl overflow-hidden bg-slate-50 border border-slate-100 shadow-sm">
              <div className="aspect-[4/3] sm:aspect-[16/10] relative">
                ...
                <Button
                  ...
                  className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-primary/90 hover:bg-primary text-white font-bold text-[9px] sm:text-[11px] uppercase tracking-widest rounded px-3 py-1.5 sm:px-4 sm:py-2 h-auto transition-all z-20"
                >
                  Download Images
                </Button>
                ...
                {images.length > 1 && (
                  <>
                    <button onClick={prevImage} className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-all z-20">
                      <ChevronLeft className="h-5 w-5 sm:h-6 sm:w-6" />
                    </button>
                    <button onClick={nextImage} className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-all z-20">
                ...
                {/* Index Counter */}
                <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 bg-black/60 text-white text-[9px] sm:text-[10px] font-bold px-2 py-0.5 sm:px-3 sm:py-1 rounded">
              ...
              <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 no-scrollbar">
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-16 sm:w-24 aspect-[4/3] flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
          ...
          <div className="lg:col-span-4 space-y-6 sm:space-y-8">
            ...
            <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl space-y-4 shadow-2xl relative overflow-hidden border border-white/10 group animate-in fade-in slide-in-from-right-8 duration-700">
               <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:rotate-12 transition-transform duration-700">
                  <Wallet className="h-16 w-16 sm:h-20 sm:w-20" />
               </div>
               <div className="relative z-10 space-y-1">
                  <div className="flex justify-between items-center">
                    <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] sm:tracking-[0.4em]">Acquisition Value</p>
                    <Badge className="bg-brand-red text-white text-[8px] sm:text-[9px] font-bold px-2 py-0.5 rounded border-none shadow-lg">VERIFIED</Badge>
                  </div>
                  <div className="flex items-baseline gap-2 pt-2">
                    <span className="text-xs sm:text-sm font-bold text-slate-400 uppercase tracking-widest">KSh</span>
                    <h2 className="text-3xl sm:text-5xl font-black tracking-tighter text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
                      {car.price.toLocaleString()}
                    </h2>
                  </div>
               ...
               <div className="pt-4 border-t border-white/10 relative z-10">
                  <div className="flex items-center gap-3 text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
               {[
                 ...
                 <div key={i} className="bg-slate-50 p-3 sm:p-4 rounded-xl flex flex-col items-center text-center space-y-2 border border-slate-100">
                    <item.icon className={`h-4 w-4 sm:h-5 sm:w-5 ${item.color}`} />
                    <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.label}</p>
                    <p className="text-xs font-black text-slate-900 uppercase truncate w-full">{item.val}</p>
                 </div>
               ))}
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-xs sm:text-sm font-black uppercase tracking-[0.2em] text-slate-900 border-b border-slate-100 pb-2">Vehicle Details</h3>
              <div className="grid grid-cols-1 gap-y-3">
                 {[
                   ...
                   <div key={i} className="flex justify-between text-[10px] sm:text-[11px] font-bold uppercase tracking-tight">
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              <Button
                className="w-full h-12 sm:h-14 bg-[#25D366] hover:bg-[#20ba54] text-white font-black text-[10px] sm:text-[12px] uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 sm:gap-3 border-none shadow-md transition-all whitespace-normal text-center py-2 relative overflow-hidden group"
                asChild
              >
                <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
                   <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                   <svg viewBox="0 0 24 24" className="h-4 w-4 sm:h-5 sm:w-5 fill-white flex-shrink-0" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span className="leading-none text-left">WhatsApp Inquiry - {car.yard_location?.split(',')[0] || 'Nairobi'}</span>
                </a>
              </Button>
              <Button
                onClick={handleDownloadImage}
                className="w-full h-12 sm:h-14 bg-blue-600 hover:bg-blue-700 text-white font-black text-[10px] sm:text-[12px] uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 sm:gap-3 border-none shadow-md transition-all"
              >
