...
const ContentCommentSection = ({ contentId, contentType }: ContentCommentSectionProps) => {
  const { user } = useAuth();
  ...
  const fetchCommentCount = async () => {
    const { count } = await supabase.from("content_comments").select("*", { count: "exact", head: true })
  ...
  const handleReply = async (parentId: string) => {
    ...
    try {
      const { error } = await supabase.from("content_comments").insert({
        ...
        content_type: contentType,
        user_id: user?.id || null,
        display_name: user ? displayName || "User" : "Guest",
    ...
    <div key={c.id} className={`flex gap-2 text-xs ${isReply ? "ml-6 mt-1.5" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-semibold">{c.is_anonymous ? "Anonymous" : getFirstName(c.display_name)}</span>
        ...
        {replyTo === c.id && (
          <div className="flex gap-1 mt-1">
            <Input placeholder="Write a reply..." value={replyText} onChange={(e) => setReplyText(e.target.value)} className="h-6 text-xs flex-1" onKeyDown={(e) => e.key === "Enter" && handleReply(c.id)} />
        ...
        {c.replies && c.replies.length > 0 && (
          <div className="space-y-1.5 mt-1">{c.replies.map((r) => renderComment(r, true))}</div>
        )}
      </div>
    </div>
  );
      ...
      {showComments && (
        <div className="mt-2 border-t border-border pt-3 space-y-3" onClick={(e) => e.preventDefault()}>
          <div className="space-y-2 bg-muted/30 rounded-lg p-3">
            <Textarea placeholder="Write your comment or message..." value={commentText} onChange={(e) => setCommentText(e.target.value)} className="text-xs min-h-[60px] resize-none" />
            <div className="flex items-center gap-1.5 mb-2">
              <Checkbox id={`anon-${contentId}`} checked={isAnonymous} onCheckedChange={(v) => setIsAnonymous(v as boolean)} className="h-3.5 w-3.5" />
              <label htmlFor={`anon-${contentId}`} className="text-xs text-muted-foreground cursor-pointer">Comment anonymously</label>
            </div>
            {!isAnonymous && (
