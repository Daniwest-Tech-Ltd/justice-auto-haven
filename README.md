...
```mermaid
flowchart TD
A[CEO / Founder] --> B[Sales Department]
A --> C[HR Department]
A --> D[Tech Division]
A --> E[Logistics Division]

B --> B1[Sales Team]
B --> B2[Customer Management]
B --> B3[Finance & Deals]
...
D --> D1[Frontend Dev]
D --> D2[Backend Dev]
D --> D3[DevOps & Security]

E --> E1[Vehicle Rentals]
E --> E2[Fleet Management]
E --> E3[Drivers & Operations]
```
...
```mermaid
flowchart TD
A[User Access] --> B{Select Portal}

B --> C[Customer Portal]
B --> D[Staff Portal]
B --> E[Admin Portal]
B --> F[Super Admin Portal]
B --> G[HR Portal]

C --> C1[Browse Vehicles]
C1 --> C2[Place Order]
C2 --> C3[Make Payment]
C3 --> C4[Receive Confirmation]

D --> D1[Manage Customers]
D1 --> D2[Process Sales]

E --> E1[System Monitoring]
E1 --> E2[Analytics Dashboard]

F --> F1[Full System Control]
F1 --> F2[Security Management]

G --> G1[Employee Records]
G1 --> G2[Payroll & Attendance]
```
...
```mermaid
flowchart LR
A[Add Vehicle] --> B[Update Catalogue]
B --> C[Display to Customers]
...
```mermaid
flowchart TD
A[Lead Generation] --> B[Customer Contact]
B --> C[Qualification]
C --> D[Vehicle Offer]
D --> E{Interested?}
...
D --> F[Activity Monitoring]
F --> G[Threat Detection]
G --> H[Auto Response System]
```
