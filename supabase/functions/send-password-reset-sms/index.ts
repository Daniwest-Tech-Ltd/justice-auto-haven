...
serve(async (req) => {
  ...
  try {
    ...
    const { phone, userId }: PasswordResetRequest = await req.json();
    ...
    const message = `Your Justice Ultimate Automobiles password reset code is: ${resetCode}. Valid for ${expiryMinutes} minutes. If you didn't request this, ignore this message.`;
    ...
    if (!brevoResponse.ok) {
      throw new Error(brevoData.message || "Failed to send password reset SMS");
    }
